 ...
 * Uses the S3 presigned-URL pipeline via optimizeImage + uploadToS3.
 */
export const handleImageUpload = async (
  ...
  // Lazy-import to keep this module light and avoid issues in SSR contexts
  const { optimizeImage } = await import("@/lib/utils/image-processing")
  ...
  const result = await uploadToS3({ file: optimizedFile, prefix: "blog-images" })
